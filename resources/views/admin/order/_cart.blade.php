<div class="row">
  <div class="col-md-12">
    <table class="table table-sripe">
      <tbody id="items">
        <tr id='empty-cart' style="display: {{ (isset($cart) && count($cart->products) > 0) ? 'none' : 'table-row' }}"><td colspan="5">{{ trans('help.empty_cart') }}</td></tr>
          ...
          @foreach($cart->products as $item )
            ...
            <tr id="{{ $id }}">
              ...
              <td class="nopadding-right" width="55%">
                ...
                {{ Form::hidden("cart[".$i."][product_id]", $id) }}
              ...
              <td class="nopadding-right" width="15%">
                {{ Form::number("cart[".$i."][unit_price]", get_formated_decimal($item->price), ['id' => 'price-'.$id, 'class' => 'form-control itemPrice no-border', 'step' => 'any', 'required']) }}
              </td>
              <td>x</td>
              ...
              <td class="nopadding-right text-center" width="10%">{{ get_formated_currency_symbol() }}
                <span id="total-{{$id}}"  class="itemTotal">
                  {{ get_formated_decimal($item->pivot->quantity * $item->price) }}
                </span>
              </td>
