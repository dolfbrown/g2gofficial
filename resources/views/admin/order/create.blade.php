...
@section('content')
  @php
    ...
    $shipping_zone = $shipping_address ?
                    get_shipping_zone_of($shipping_address->country_id, $shipping_address->state_id) : Null;
            ...
            <div class="row">
              ...
              <div class="col-md-6" id="summary-block">
                <table class="table">
                  ...
                  @if( (bool) get_formated_decimal(config('system_settings.order_handling_cost')) )
                    <tr>
                      <td class="text-right">{{ trans('app.handling') }}</td>
                      <td class="text-right" width="40%">{{ get_formated_currency_symbol() }}
                        <span id="summary-handling">{{ get_formated_decimal(config('system_settings.order_handling_cost'), true, 2) }}</span>
                      </td>
                    </tr>
                  @endif
        ...
        <div class="box">
          ...
          <div class="box-body">
            <div class="form-group">
              ...
              {!! Form::select('payment_method_id', $payment_methods ,  (isset($cart->payment_method_id)) ? $cart->payment_method_id : null, ['class' => 'form-control select2-normal', 'placeholder' => trans('app.placeholder.payment'), 'required']) !!}
  ...
  <script language="javascript" type="text/javascript">
    ;(function($, window, document) {
      ...
      if (payment_methods.length == 0) {
        $("#global-alert-msg").html('{!! trans('messages.notice.no_active_payment_method') . ' ' . '<a href="' . route('admin.setting.config.payment_method') . '">' . trans('app.activate') . '</a>' !!}');
      ...
      var shipping_zone_tax_id = <?=optional($shipping_zone)->tax_id ? $shipping_zone->tax_id : 'NaN';?>;

      if ( shipping_zone_tax_id ) {
        setTax(shipping_zone_tax_id);
      }
          ...
          function() {
              ...
              var node = '<tr id="'+ ID +'">' +
                  ...
                  '<td class="nopadding-right" width="55%">' + itemDescription +
                    '<input type="hidden" name="cart['+ numOfRows +'][product_id]" value="'+ ID +'"></input>' +
