...
if ( ! function_exists('setLastUpdateCheckAt') )
{
    /**
     * Set setLastUpdateCheckAt the config
     */
    function setLastUpdateCheckAt()
    {
        $setting = System::find(1);
        $setting->last_update_check = Carbon::now();
        $setting->save();
    }
}
...
if ( ! function_exists('saveOrderFromCart') )
{
    ...
    function saveOrderFromCart($request, $cart)
    {
        ...
        $cart_items = $cart->products->pluck('pivot');
        $order_items = [];
...
if ( ! function_exists('revertOrderAndMoveToCart') )
{
    ...
    function revertOrderAndMoveToCart($order)
    {
        ...
        // Add order item into pivot table
        $order_items = $order->products->pluck('pivot');
        $cart_items = [];
...
if ( ! function_exists('prepareFilteredListings') )
{
    ...
    function prepareFilteredListings($request, $categoryGroup)
    {
        ...
        foreach ($categoryGroup->categories as $t_category) {
            $t_products = $t_category->products()->available()->filter($request->all())
...
if ( ! function_exists('crosscheckAndUpdateOldCartInfo') )
{
    ...
    function crosscheckAndUpdateOldCartInfo($request, Cart $cart)
    {
        ...
        $discount = Null;
        $shipping_weight = 0;
        $handling = config('system_settings.order_handling_cost');
        ...
        foreach ($cart->products as $item) {
            $temp_qtt = $request->quantity ? $request->quantity[$item->pivot->id] : $item->pivot->quantity;
            ...
            // Prepare and Update the item into pivot table
            $pivot_data = [
                'quantity' => $temp_qtt,
            ...
            DB::table('cart_items')->where('id', $item->pivot->id)->update($pivot_data);

            // $cart->products()->updateExistingPivot($item->pivot->id, ['quantity' => $temp_qtt, 'unit_price' => $unit_price]);
        }
