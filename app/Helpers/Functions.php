...
{
    /**
     * Return ptatform title or the application title
     */
...
if ( ! function_exists('get_product_img_src') )
{
    ...
    {
        if (is_numeric($item) && !($item instanceof \App\Product)) {
            $item = \App\Product::withTrashed()->findOrFail($item);
        }

        if ($item->featuredImage) {
            return get_storage_file_url($item->featuredImage->path, $size);
        }
...
if ( ! function_exists('get_variant_img_src') )
{
    function get_variant_img_src($item = null, $size = 'medium', $type = 'primary')
    {
        if (is_numeric($item) && !($item instanceof \App\ProductVariant)) {
            $item = \App\ProductVariant::findOrFail($item);
        }
        ...
        return get_product_img_src($item->product, $size, $type);
    }
}
...
if ( ! function_exists('get_formated_order_number') )
{
    function get_formated_order_number($order_id = Null)
    {
        ...
        return config('system_settings.order_number_prefix') . $order_id . config('system_settings.order_number_suffix');
    }
}
...
{
    /**
     * Return the shipping zone id of given country and state
     *
     * @param $tax
     */
    function get_shipping_zone_of($country, $state = null) {
        ...
        foreach ($zones as $zone) {
            // Check has a worldwide shipping zone
...
{
    /**
     * Get shipping rates list for the given zone.
     */
    function getShippingRates($zone = Null)
    {
        ...
        if( ! Auth::guard('web')->check() ) return new stdClass();
...
{
    /**
     * Return default packaging
     */
    function getDefaultPackaging()
    {
...
if ( ! function_exists('getPackagings') )
{
    ...
    {
        return \DB::table('packagings')->select('id', 'name', 'cost')->where('active', 1)->whereNull('deleted_at')->get();
    }
}
        ...
        switch ($id) {
            case \App\PaymentMethod::TYPE_PAYPAL:
                return [
                        ...
                        'provider' => trans('app.payment_method_type.paypal.provider'),
                    ];
            ...
            case \App\PaymentMethod::TYPE_CREDIT_CARD:
                return [
                        ...
                        'provider' => trans('app.payment_method_type.credit_card.provider'),
                    ];
            ...
            case \App\PaymentMethod::TYPE_MANUAL:
                return [
                        ...
                        'provider' => trans('app.payment_method_type.manual.provider'),
                    ];
        }
    }
}
