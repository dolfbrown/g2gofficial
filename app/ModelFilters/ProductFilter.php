...
class ProductFilter extends ModelFilter
{
    ...
    */
    ...
    {
        return $this->where('available_from', '>', Carbon::now()->subDays(config('system.filter.new_arrival', 7)));
    }
    ...
    public function sortBy($sort_by)
    {
        switch ($sort_by) {
            case 'newest':
                return $this->orderBy('available_from', 'desc');
            case 'oldest':
                return $this->orderBy('available_from', 'asc');
            case 'price_acs':
