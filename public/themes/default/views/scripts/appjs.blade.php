...
;(function($, window, document) {
    $(document).ready(function(){
        ...
        $("#buy-now-btn").on("click", function(e) {
            var buyNowUrl = $("#buy-now-btn").data('href');
            var variant = $("#variant-id").val();

            if(variant != 'Null') {
                buyNowUrl = buyNowUrl + '?variantId=' + variant;
            }

            apply_busy_filter('body');

            window.location = buyNowUrl;
        });
    ...
    function initAppPlugins()
    {
        ...
        $(".sc-add-to-cart").on("click", function(e) {
            e.preventDefault();
            var itemContainer = $(this).closest('.sc-product-item');
            var qtt = itemContainer.find('input.product-info-qty-input').val();
            var shipTo = itemContainer.find('select#shipTo').val();
            var shippingZoneId = itemContainer.find('input#shipping-zone-id').val();
            var shippingRateId = itemContainer.find('input#shipping-rate-id').val();
            var variantId = itemContainer.find('input#variant-id').val();
            ...
            $.ajax({
                ...
                complete: function (xhr, textStatus) {
                    ...
                    if(200 == xhr.status){
                        ...
                        @include('layouts.notification', ['message' => trans('theme.item_not_found'), 'type' => 'warning', 'icon' => 'times-circle'])
                    }
                    else if(307 == xhr.status){
                        var tempUrl = "{{route('show.product', '::ITEMSLUG::')}}";
                        var fullUrl = tempUrl.replace("::ITEMSLUG::", itemContainer.data('slug'));
                        ...
                        @include('layouts.notification', ['message' => trans('theme.select_options'), 'type' => 'info', 'icon' => 'info-circle'])
                    }
                    else{
