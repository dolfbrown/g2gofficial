...
;(function($, window, document) {
    $(document).ready(function(){
    	...
    	$('.shopping-cart-table-wrap').each(function(e){
			...
			// Skip shippingzone checking for downlodable cart
			if($(this).data('downlodable')) {
        ...
        $(".ship_to").on('change', function(e) {
		    ...
		    var zone = getFromPHPHelper('get_shipping_zone_of', [countryId]);
    	...
    	$('.apply_seller_coupon').on('click', function(e){
	        ...
	        if(coupon){
				$.ajax({
				    ...
				    type: 'POST',
				    data: {'coupon':coupon,'cart':cart,'zone':zone},
				    dataType: 'JSON',
