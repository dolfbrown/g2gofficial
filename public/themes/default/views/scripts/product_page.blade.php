...
;(function($, window, document) {
    ...
    var handlingCost = {{ config('system_settings.order_handling_cost') ?? 0 }};
    var unitPrice = {{ $item->currnt_price() }};
    var variants = '{!! $item->variants !!}';
    {{-- // var variants = JSON.parse('{!! $item->variants !!}'); --}}
    ...
    function setSelectedVariantInfo()
    {
        apply_busy_filter('body');
        $('#loading').show();
        ...
        if(filtered == undefined) {
            ...
            // If the item has attrs set out of stock
            if (attrs.length > 0) {
                setOutOfStock();
    ...
    $('#shipTo').on('change', function(){
        ...
        var zone = getFromPHPHelper('get_shipping_zone_of', [$(this).val()]);
    ...
    function setSalePrice(item)
    {
        if(
            ...
            // itemWrapper.find('.old-price').show().html(getFormatedPrice(item.price).replace(/\.?0+$/, ''));
            ...
            // itemWrapper.find('.product-info-price-new').html(getFormatedPrice(item.price).replace(/\.?0+$/, ''));
        }
    }
    ...
    function setStockQuantity(item)
    {
        ...
        itemWrapper.find('.product-info-title').html(title);
