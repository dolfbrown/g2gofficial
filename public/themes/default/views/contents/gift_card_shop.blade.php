<div class="row product-list">
    @foreach($giftCards as $item)
        <div class="col-md-3">
            <div class="product product-grid-view sc-product-item" data-slug="{{$item->slug}}">
                <input name="product_price" value="{{ get_formated_decimal($item->currnt_price(), true, 2) }}" type="hidden"/>
                ...
                {{-- <ul class="product-info-labels">
                    ...
                    @endif
                    @if($item->hasOffer())
                        <li>@lang('theme.percent_off', ['value' => get_percentage_of($item->price, $item->offer_price)])</li>
                    @endif
                </ul> --}}
